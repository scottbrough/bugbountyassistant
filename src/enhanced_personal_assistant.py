...
class EnhancedBugBountyAssistant:
    ...
    def initialize_hunt(self, target: str, platform: Optional[str] = None, program_handle: Optional[str] = None):
        """Initialize enhanced hunt with platform integration"""
        self.target = target or ''
        self.workspace = Path(f"hunt_{self.target.replace('.', '_').replace(':', '_')}_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
        ...
        self.session_data = {
            ...
            "program_handle": program_handle or '',
    ...
    def ai_target_analysis(self) -> Dict:
        ...
        try:
            ...
            if content and isinstance(content, str):
                ...
                analysis = {"error": "No content returned from AI"}
            
            # Fix: assign program_insights directly as a key
            if self.program_info and isinstance(analysis, dict):
    ...
    def intelligent_recon(self) -> Dict:
        ...
        for target in top_targets:
            if not target:
                continue
            endpoints = self._discover_content(str(target))
    ...
    def ai_report_generation(self) -> Dict:
        ...
        if self.program_info and self.program_info.get('platform') and self.workspace:
...
<body>
    <div class=\"container\">
        ...
        {''.join([f'<div class="finding severity-{finding.get("severity", "unknown")}"><h3>{finding.get("type", "Unknown Vulnerability")}</h3><p><strong>URL:</strong> {finding.get("url", "Unknown")}</p><p><strong>Severity:</strong> {finding.get("severity", "Unknown").title()}</p><p><strong>Discovery Method:</strong> {finding.get("discovery_method", "Unknown")}</p>' + (f'<p><strong>WAF Detected:</strong> {finding.get("waf_info", {}).get("type", "None")}</p>' if finding.get('waf_detected') else '') + (f'<p><strong>Evasion Technique:</strong> {finding.get("evasion_technique")}</p>' if finding.get('evasion_technique') else '') + f'<div class="code">{finding.get("evidence", "No evidence available")}</div></div>' for finding in self.findings]) if self.findings else '<p>No vulnerabilities found.</p>'}
    ...
    def _generate_enhanced_exploit(self, finding: Dict) -> Dict:
        ...
        try:
            ...
            if content and isinstance(content, str):
                ...
                exploit_content = "No exploit content returned from AI"
            return {
...
"""
    ...
    def _generate_enhanced_poc_scripts(self, exploits: Dict):
        ...
        if not self.workspace:
            logger.warning("Workspace is not set. Cannot generate PoC scripts.")
            return
    ...
    def _find_subdomains(self) -> List[str]:
        ...
        try:
            result = subprocess.run(
                ["subfinder", "-d", str(self.target), "-silent"],
        ...
        try:
            result = subprocess.run(
                ["amass", "enum", "-passive", "-d", str(self.target)],
                capture_output=True, text=True, timeout=300
            )
            if result.returncode == 0:
                subdomains.update(result.stdout.strip().split('\n'))
